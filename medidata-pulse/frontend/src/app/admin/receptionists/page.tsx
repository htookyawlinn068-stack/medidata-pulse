...
export default function AddReceptionistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/admin/receptionists', {
        method: 'POST',
        headers: {
      ...
      if (res.ok && data.success) {
        ...
        throw new Error(data.message || 'Failed to add receptionist');
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-900 text-slate-100 p-8 items-center justify-center font-sans">
      <div className="max-w-md w-full bg-slate-950 p-8 rounded-2xl border border-slate-800">
        <h1 className="text-xl font-bold text-white mb-1">Add New Receptionist Account</h1>
        <p className="text-xs text-slate-400 mb-6">လက်ခံဝန်ထမ်း (Receptionist) အတွက် အကောင့်အသစ် ဖန်တီးပေးပါ။</p>

        {error && <div className="mb-4 bg-red-950/50 border border-red-800 text-red-400 p-3 rounded-xl text-xs">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Receptionist Name</label>
            <input 
              type="text" 
              name="name" 
              required 
